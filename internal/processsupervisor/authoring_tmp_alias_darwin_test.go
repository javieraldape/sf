...
func TestAuthoringSandboxProductionTemporaryAlias(t *testing.T) {
	...
	for _, alias := range []bool{false, true} {
		...
		t.Run(name, func(t *testing.T) {
			...
			cmd.Args[0] = prefix[1]
			cmd.Dir = temporary
			cmd.Env = append(env, "CLAUDE_CODE_TMPDIR="+temporary, "SF_AUTHORING_TMP_ALIAS_HELPER=1", "SF_AUTHORING_TMP_ALIAS_OUTSIDE="+outside)
			...
			runErr := cmd.Run()
			if runErr != nil || stdout.truncated || stderr.truncated || !bytes.Contains(stdout.Bytes(), []byte("authoring-tmp-private-ok")) || !bytes.Contains(stdout.Bytes(), []byte("authoring-internal-tmp-ok")) {
				t.Fatalf("native temporary boundary failed: alias=%t exit_ok=%t timed_out=%t stdout_present=%t stderr_present=%t truncated=%t home_ok=%t tmp_ok=%t internal_tmp_ok=%t outside_denied=%t child_denied=%t", alias, runErr == nil, ctx.Err() != nil, stdout.Len() > 0, stderr.Len() > 0, stdout.truncated || stderr.truncated, bytes.Contains(stdout.Bytes(), []byte("authoring-home-operations-ok")), bytes.Contains(stdout.Bytes(), []byte("authoring-tmp-operations-ok")), bytes.Contains(stdout.Bytes(), []byte("authoring-internal-tmp-ok")), bytes.Contains(stdout.Bytes(), []byte("authoring-outside-denied")), bytes.Contains(stdout.Bytes(), []byte("authoring-child-denied")))
			}
		})
	}
}
...
func authoringTemporaryAliasHelper(t *testing.T) {
	t.Helper()
	internal := os.Getenv("CLAUDE_CODE_TMPDIR")
	if internal == "" || internal != os.Getenv("TMPDIR") {
		t.Fatal("internal temp not bound to private temp")
	}
	directory := filepath.Join(internal, "claude-fixedfixture")
	if os.Mkdir(directory, 0700) != nil {
		t.Fatal("private internal directory denied")
	}
	file := filepath.Join(directory, "probe")
	if os.WriteFile(file, []byte("fixed-internal-payload"), 0600) != nil {
		t.Fatal("private internal write denied")
	}
	contents, err := os.ReadFile(file)
	if err != nil || string(contents) != "fixed-internal-payload" || os.Remove(file) != nil || os.Remove(directory) != nil {
		t.Fatal("private internal read/delete denied")
	}
	fmt.Fprintln(os.Stdout, "authoring-internal-tmp-ok")
