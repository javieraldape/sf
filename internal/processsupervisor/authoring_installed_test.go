...
func TestInstalledClaudeAuthoringPurposes(t *testing.T) {
	...
	for _, test := range []struct{ purpose, prompt string }{
		...
		if runErr != nil || !proofValid {
			...
			t.Fatalf("authoring failure: stage=%s exit_observed=%t exit_code=%d signal=%d capture_known=%t stdout_present=%t stderr_present=%t stdout_truncated=%t stderr_truncated=%t process_reported_hint=%s process_reported_error_family=%s (advisory only; not origin/API proof) proof_valid=%t launch_count=%d; no retry attempted", d.Stage, d.ExitObserved, d.ExitCode, d.Signal, d.CaptureKnown, d.StdoutPresent, d.StderrPresent, d.StdoutTruncated, d.StderrTruncated, d.ProcessReportedHint, d.ProcessReportedErrorFamily, proofValid, launches)
		}
