...
func authoringPolicyDigest() string {
	return contracts.AuthoringDigest([]byte("sf.authoring.claude/v3:2.1.263:private-cwd:empty-tools:empty-mcp:subscription-oauth:restricted:safe-mode:json-schema:no-persistence:max-turns3:structured-retries1:90s:64KiB:private-filesystem:private-internal-tmp\x00" + authoring.Schema + "\x00" + authoring.Instruction + "\x00" + authoring.HomeSchema + "\x00" + authoring.HomeInstruction))
}
