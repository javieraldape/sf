...
func (s *Supervisor) RunAuthoring(ctx context.Context, claim contracts.AuthoringClaim, input contracts.AuthoringInput, record func(context.Context, contracts.ProviderLaunch) error) (result contracts.AuthoringResult, proof contracts.AuthoringProof, failure error) {
	diagnostic := AuthoringRunDiagnostic{Stage: "claim"}
	defer func() {
		if failure != nil {
			failure = &authoringRunError{diagnostic: diagnostic, cause: failure}
		}
	}()
	...
	diagnostic.Stage = "sandbox"
	prefix, err := command(trusted, home, tmp)
	if err != nil {
		return noLaunch(err)
	}
	diagnostic.Stage = "gate_setup"
	self := s.Executable
	if self == "" {
	...
	// cmd.Wait has returned and both captures are stable at this boundary.
	diagnostic.CaptureKnown = true
	if cmd.ProcessState != nil {
		diagnostic.ExitObserved = true
		diagnostic.ExitCode = cmd.ProcessState.ExitCode()
		if status, ok := cmd.ProcessState.Sys().(syscall.WaitStatus); ok && status.Signaled() {
			diagnostic.Signal = int(status.Signal())
		}
	}
	...
	if launchErr != nil || err != nil || stdout.truncated || stderr.truncated {
		if launchErr == nil {
			if err != nil {
				if diagnostic.Stage != "cancel" {
					diagnostic.Stage = "process_exit"
				}
			} else {
	...
	var isError *bool
	diagnostic.Stage = "result_envelope"
